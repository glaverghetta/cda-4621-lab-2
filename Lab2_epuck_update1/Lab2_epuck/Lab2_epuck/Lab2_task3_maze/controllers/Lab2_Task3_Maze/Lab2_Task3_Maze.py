...
MINIMUM_LINEAR_SPEED = 4.5    # Minimum forward speed, in in/s, at which the robot should travel.
...
while robot.step(timestep) != -1:
    if mToIn(fs.getValue()) <= STOPPING_DISTANCE and mToIn(rs.getValue()) <= STOPPING_DISTANCE + 1 and mToIn(ls.getValue()) <= STOPPING_DISTANCE + 1:
        lm.setVelocity(0)
        rm.setVelocity(0)
        break
    ...
    if robotState == "wall_follow":
        ...
        if mToIn(fs.getValue()) <= STOPPING_DISTANCE:
            ...
            if nextTurnState == "turn_right":
            
                if mToIn(rs.getValue()) <= STOPPING_DISTANCE + 1:
                    num180Turns += 1
                    if num180Turns % 2 == 0:
                        nextTurnState = "turn_left"
                ...
                if mToIn(ls.getValue()) <= STOPPING_DISTANCE + 1:
                    num180Turns += 1
                    if num180Turns % 2 == 0:
                        nextTurnState = "turn_right"
                    robotState = "turn_around"
